...
def generatePtn(p,max_index,df_current):
	...
	for i in list_index:
		...
		list_points.append([IDs,lat,lon,total,timestamp_start,timestamp_end,diameter])
...
def unique(filename,inputDirectory,p,spatialResolution,temporalResolution,iterations,outputDirectory):	
	...
	df_current=pd.read_csv(inputDirectory+'/'+filename,names=['IDs','lat','lon','total','timestampStart','timestampEnd','diameter'])
	max_index= df_current.shape[0]
	# read prealably the other user df and put them in a tab :
	list_df_users=[]
	for user in users:
		df=pd.read_csv(inputDirectory+'/'+user,names=['IDs','lat','lon','total','timestampStart','timestampEnd','diameter'])
