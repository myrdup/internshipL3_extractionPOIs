...
inputDirectory= sys.argv[1] # dataset where for each user, a file containing POIs ('IDs','lat','lon','total','timestampStart','timestampEnd')
...
def generatePtn(p,max_index,df_current):
	...
	for i in list_index:
		...
		list_points.append([IDs,lat,lon,total,timestamp_start,timestamp_end])
...
def unique(filename,inputDirectory,p,spatialResolution,temporalResolution,iterations,outputDirectory):	
	...
	df_current=pd.read_csv(inputDirectory+'/'+filename,names=['IDs','lat','lon','total','timestampStart','timestampEnd'])
	max_index= df_current.shape[0]
	# read prealably the other user df and put them in a tab :
	list_df_users=[]
	for user in users:
		df=pd.read_csv(inputDirectory+'/'+user,names=['IDs','lat','lon','total','timestampStart','timestampEnd'])
